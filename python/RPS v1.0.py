...
class CyclePlayer(Player):  # To Make the game more chanllenging!
    def __init__(self):
        Player.__init__(self)
        self.my_move = None
...
class Game:  # Game Engine!
    ...
    def play_round(self):
        ...
        print(f"John played {move2}")  # Introducing John The RPS AI!
    ...
    def play_game(self):
        try:
            # Welcome Message
            print("Welcome to Rock, Paper, Scissors CLI minigame!")
            time.sleep(1)
            print("John Challenges You to a game of RPS!")  # John Dialogue
            time.sleep(1)
            prompt = input("Do You Accept the Challenge?\n" +
                           "Press Y to Accept X to Exit.\n")
            if prompt in ("x", "X"):
                print("Goodbye then!")
                exit()
            elif prompt is "y":
        ...
        except KeyboardInterrupt:  # Handle errors Like a Beast!
